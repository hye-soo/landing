      ...
      <div className="container mx-auto max-w-6xl px-4 md:px-6">
        ...
        >
            ...
            <div className="space-y-6">
              {badgeText && (
                <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
                  <Star className="mr-1 h-3.5 w-3.5 fill-indigo-400" />
                  <span>{badgeText}</span>
                </div>
              )}

              <h2 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl text-slate-900">{title}</h2>

              <p className="max-w-[600px] text-slate-500 md:text-xl/relaxed">{subtitle}</p>
                ...
                {testimonials.map((_, index) => (
                  <button
                    ...
                    className={`h-2.5 rounded-full transition-all duration-300 ${
                      activeIndex === index ? "w-10 bg-indigo-500" : "w-2.5 bg-slate-300 hover:bg-slate-400"
                    }`}
          ...
          <motion.div variants={itemVariants} className="relative h-full mr-10 min-h-[300px] md:min-h-[400px]">
            {testimonials.map((testimonial, index) => (
              ...
              >
                <div className="bg-white/60 backdrop-blur-xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.06)] rounded-2xl p-8 h-full flex flex-col">
                  ...
                  <div className="relative mb-6 flex-1">
                    <Quote className="absolute -top-2 -left-2 h-8 w-8 text-indigo-500/20 rotate-180" />
                  ...
                  <div className="flex items-center gap-4">
                    <Avatar className="h-12 w-12 border border-white/60 shadow-sm">
                      ...
                      <AvatarFallback className="bg-slate-100 text-slate-600">{testimonial.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
            ...
            {/* Decorative elements */}
            <div className="absolute -bottom-6 -left-6 h-24 w-24 rounded-xl bg-indigo-500/10 blur-xl"></div>
            <div className="absolute -top-6 -right-6 h-24 w-24 rounded-xl bg-rose-500/10 blur-xl"></div>
          </motion.div>
        </motion.div>
        ...
        {trustedCompanies.length > 0 && (
          <motion.div variants={itemVariants} initial="hidden" animate={controls} className="mt-24 text-center">
            <h3 className="text-sm font-medium text-slate-400 mb-8">{trustedCompaniesTitle}</h3>
